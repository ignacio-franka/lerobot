...
class LeRobotDataset(torch.utils.data.Dataset):
            ...
            if self.features[key]["dtype"] == "image":
                ...
                if frame_index == 0:
                    ...
                    if video_path.is_file():
                        ...
                        continue
                    self.video_encoders[key] = get_video_encoder(
                ...
                if key in self.video_encoders:
                    self.video_encoders[key].put(frame[key])
            else:
    ...
    def save_episode(self, episode_data: dict | None = None) -> None:
        ...
        if len(self.meta.video_keys) > 0:
            for key in self.meta.video_keys:
                if key in self.video_encoders:
                    # A None frame is sent to the video encoder to close the video container
                    self.video_encoders[key].put(None)
                    self.video_encoders[key].join()
                else:
                    logging.warning(
